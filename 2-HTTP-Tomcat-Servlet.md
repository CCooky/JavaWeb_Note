...
![1627031023395](images/1627031023395-16440699376001.png)
...
等学习完JavaWeb之后，同学们就可以使用Java语言开发我们上述所说的网站。而国内很多大型网站公司也是首选Java语言来解决web互联网相关的问题。那都有哪些公司的系统是使用Java语言的呢?
 ![](images/20210717183958532-16440699376012.png)
...
* 在日常上网的过程中，我们除了看到这些好看的页面以外，还会碰到很多动态内容，比如我们常见的百度登录效果:
  ![1627037814180](images/1627037814180-16440699376014.png)
  `张三`登录以后在网页的右上角看到的是 `张三`，而`李四`登录以后看到的则是`李四`。所以不同的用户访问相同的资源看到的内容大多数是不一样的，要想实现这样的效果，光靠静态资源是无法实现的。

#### 1.2.3 动态资源

* ==动态资源主要包含Servlet、JSP等，主要用来负责逻辑处理。==动态资源其实就是Java代码，而Servlet、JSP是一种编写规范。
...
* 这就涉及到了下一步——前面学习的数据库
* ![1627038674340](images/1627038674340-16440699376015.png)
...
* 整个Web的访问过程就如下图所示:
  ![1627039320220](images/1627039320220-16440699376016.png)
  ...
  其实请求的部分，其实就是建立通信连接，前面学习过了两个简单基础的通信协议，这里需要学习新的协议——HTTP
...
通过http协议建立连接之后，需要对内容进行解析，这就是接下来的WEB服务器。
...
<img src="images/image-20240704102733877.png" alt="image-20240704102733877" style="zoom:80%;" />
...
![1627058356051](images/1627058356051-164406993760118.png)
...
![1627060368806](images/1627060368806-164406993760119.png)
...
![1627176045795](images/1627176045795-164406993760121.png)
...
![1627178001030](images/1627178001030-164406993760120.png)
...
* 打开`apache-tomcat-8.5.68`目录就能看到如下目录结构，每个目录中包含的内容需要认识下,
  ...
  ![1627178815892](images/1627178815892-164406993760122.png)
...
![1627179006011](images/1627179006011-164406993760123.png)

启动后，通过浏览器访问 `http://localhost:8080`能看到Apache Tomcat的内容就说明Tomcat已经启动成功。

![1627199957728](images/1627199957728-164406993760124.png)

==注意==: 启动的过程中，控制台有中文乱码，需要修改conf/logging.prooperties

![1627199827589](images/1627199827589-164406993760125.png)
...
![1627200509883](images/1627200509883-164406993760126.png)
...
* Tomcat的端口号取值范围是0-65535之间任意未被占用的端口，如果设置的端口号被占用，启动的时候就会包如下的错误
  ...
  ![1627200780590](images/1627200780590-164406993760127.png)

* Tomcat启动的时候，启动窗口一闪而过: 需要检查JAVA_HOME环境变量是否正确配置

![1627201248802](images/1627201248802-164406993760128.png)
...
* Tomcat部署项目： 将项目放置到webapps目录下，即部署完成。
  ...
  * 通过浏览器访问`http://localhost/hello/a.html`，能看到下面的内容就说明项目已经部署成功。
    ...
    ![1627201572748](images/1627201572748-164406993760129.png)
...
* **一般JavaWeb项目会被打包称==war==包，然后将war包放到Webapps目录下，Tomcat会自动解压缩war文件**
  ...
  * 通过浏览器访问`http://localhost/haha/a.html`，能看到下面的内容就说明项目已经部署成功。
    ...
    ![1627201868752](images/1627201868752-164406993760130.png)
...
1. 创建Maven项目
   ...
   ![1627227574092](images/1627227574092-164406993760133.png)

2. 选择使用Web项目骨架

   ![1627227650406](images/1627227650406-164406993760134.png)

3. 输入Maven项目坐标创建项目

   ![1627228065007](images/1627228065007-164406993760135.png)

4. 确认Maven相关的配置信息后，完成项目创建

   ![1627228413280](images/1627228413280-164406993760136.png)

5. 删除pom.xml中多余内容，只留下面的这些内容，注意打包方式 jar和war的区别

   ![1627228584625](images/1627228584625-164406993760237.png)
...
1. 创建Maven项目
   ...
   ![1627229111549](images/1627229111549-164406993760239.png)

2. 选择不使用Web项目骨架

   ![1627229137316](images/1627229137316-164406993760240.png)

3. 输入Maven项目坐标创建项目

   ![1627229371251](images/1627229371251-164406993760241.png)

4. 在pom.xml设置打包方式为war,默认是不写代表打包方式为jar

   ![1627229428161](images/1627229428161-164406993760242.png)

5. 补齐Maven Web项目缺失webapp的目录结构

   ![1627229584134](images/1627229584134-164406993760243.png)

6. 补齐Maven Web项目缺失WEB-INF/web.xml的目录结构

   ![1627229676800](images/1627229676800-164406993760244.png)
...
#### 3.4.1 集成本地Tomcat（可以用高版本）

目标: 将刚才本地安装好的Tomcat8集成到IDEA中，完成项目部署，具体的实现步骤

1. 打开添加本地Tomcat的面板

   ![1627229992900](images/1627229992900-164406993760248.png)

2. 指定本地Tomcat的具体路径

   ![1627230313062](images/1627230313062-164406993760249.png)

3. 修改Tomcat的名称，此步骤可以不改，只是让名字看起来更有意义，HTTP port中的端口也可以进行修改，比如把8080改成80

   ![1627230366658](images/1627230366658-164406993760250.png)

4. 将开发项目部署项目到Tomcat中

   ![1627230913259](images/1627230913259-164406993760251.png)
...
5. 部署成功后，就可以启动项目，为了能更好的看到启动的效果，可以在webapp目录下添加a.html页面
   ...
   ![1627233265351](images/1627233265351-164406993760252.png)

6. 启动成功后，可以通过浏览器进行访问测试

   ![1627232743706](images/1627232743706-164406993760253.png)

7. 最终的注意事项

   ![1627232916624](images/1627232916624-164406993760254.png)

   

至此，IDEA中集成本地Tomcat进行项目部署的内容我们就介绍完了，整体步骤如下，大家需要按照流程进行部署操作练习。

![1627205657117](images/1627205657117-164406993760255.png)

#### 3.4.2 Tomcat Maven插件（版本7）
...
![1627233963315](images/1627233963315-164406993760256.png)

==注意:==

* 如果选中项目并右键点击后，看不到Run Maven和Debug Maven，这个时候就需要在IDEA中下载Maven Helper插件，具体的操作方式为: File --> Settings --> Plugins --> Maven Helper ---> Install,安装完后按照提示重启IDEA，就可以看到了。

![1627234184076](images/1627234184076-164406993760257.png)
...
## 4，Servlet（掌握HTTPServlet）

### 4.1 简介

![1627234763207](images/1627234763207-164406993760258.png)
...
* ==Servlet是JavaEE规范之一，其实就是一个接口，==将来我们需要定义Servlet类实现Servlet接口，然后给web服务器运行这个Servlet类
  ...
  ![1627234972853](images/1627234972853-164406993760259.png)
...
* 通过案例演示下上述的生命周期
  ...
  */
  ...
  ![1627239292226](images/1627239292226-164406993760261.png)
...
![1627240593506](images/1627240593506-164406993760262.png)
...
**下面的步骤是对HttpServlet的实现原理进行解释，自己创建一个这样的实现类看看**
...
* **一个Servlet,可以配置多个urlPattern**
  ...
  ![1627272805178](images/1627272805178-164406993760263.png)
...
* ==urlPattern配置规则==
  ...
  * ==精确匹配==
    ...
    ![1627273174144](images/1627273174144-164406993760264.png)
  ...
  * ==目录匹配==
    ...
    ![1627273184095](images/1627273184095-164406993760265.png)
    ...
    答案是: 能、能、demo8，进而我们可以得到的结论是`/user/*`中的`/*`代表的是零或多个层级==访问目录同时精确匹配优先级要高于目录匹配。==

  * 扩展名匹配

    ![1627273194118](images/1627273194118-164406993760266.png)
    ...
    ![1627274483755](images/1627274483755-164406993760267.png)

    2. 如果路径配置的是`*.do`,那么在*.do的前面不能加`/`,否则会报错

    ![1627274368245](images/1627274368245-164406993760268.png)

  * 任意匹配

    ![1627273201370](images/1627273201370-164406993760269.png)
