...
![1628735746602](images/1628735746602.png)
...
![1628738273049](images/1628738273049.png)
...
![1628741839475](images/1628741839475.png)

所以ServletRequest和HttpServletRequest是继承关系，并且两个都是接口，接口是无法创建对象，这个时候就引发了下面这个问题:

![1628742224589](images/1628742224589.png)

==这个时候，我们就需要用到Request继承体系中的`RequestFacade`:==

* 该类实现了HttpServletRequest接口，也间接实现了ServletRequest接口。
* 因为Servlet类中的service方法、doGet方法或者是doPost方法最终都是由**Web服务器[Tomcat]来调用的**，所以Tomcat提供了方法参数接口的具体实现类，并完成了对象的创建.
...
![1628748240075](images/1628748240075.png)

对于这三部分内容，**request对象**都提供了对应的API方法来获取，具体如下:
...
![1628762794935](images/1628762794935.png)

#### 2.2.2 获取请求头数据

对于请求头的数据，格式为`key: value`如下:

![1628768652535](images/1628768652535.png)
...
![1628768665185](images/1628768665185.png)
...
![1628770516387](images/1628770516387.png)
...
![1628776252445](images/1628776252445.png)

**对于这样问题，request对象单独提供了专门获取请求参数的方法，其实就是对getQueryString()和 getReader()又进行了一次封装（新建方法，里面调用这两个方法，拿到数据，然后整理好，返回给我们程序员，舒舒服服），让我们可以更加方便的获取请求参数**

以后只需要调用request提供的方法即可，这些增强的获取请求参数的方法实现的原理，是这样的：

(1) 根据不同的请求方式获取请求参数，获取的内容如下:

![1628778931277](images/1628778931277.png)

(2 ) 把获取到的内容进行分割，内容如下:

![1628779067793](images/1628779067793.png)
...
![1628780937599](images/1628780937599.png)
...
![1628780965283](images/1628780965283.png)
...
![1628781031437](images/1628781031437.png)
...
![1628781176434](images/1628781176434.png)
...
![1628781419752](images/1628781419752.png)

由于格式固定，所以我们可以使用IDEA提供的模板来制作一个Servlet的模板。IDEA流批哦。

(1) 按照自己的需求，修改Servlet创建的模板内容

![1628781545912](images/1628781545912.png)

（2）使用servlet模板创建Servlet类

![1628782117420](images/1628782117420.png)
...
![image-20220207132907228](images/image-20220207132907228.png)
...
![1628857213364](images/1628857213364.png)
...
* 浏览器地址栏路径不发生变化
  ...
  ![1628857365153](images/1628857365153.png)
...
![1628857632899](images/1628857632899.png)
...
![1628857761317](images/1628857761317.png)
...
![1628858926498](images/1628858926498.png)
...
![1628859051368](images/1628859051368.png)
...
![1628859268095](images/1628859268095.png)
...
![1628861030429](images/1628861030429.png)
...
![1628861404699](images/1628861404699.png)
...
* 浏览器地址栏路径发送变化
  ...
  当进行重定向访问的时候，由浏览器发送的两次请求，所以地址会发生变化
...
![1628862170296](images/1628862170296.png)
...
![1628862652700](images/1628862652700.png)
...
对于转发来说，是在服务端进行的，服务端已经知道我们当前的项目路径

对于重定向来说，是由浏览器来发送了两次请求，浏览器可不晓得路径是什么
...
![1628863270545](images/1628863270545.png)
...
![1628863905362](images/1628863905362.png)
...
![1628864140820](images/1628864140820.png)
...
![1628864390263](images/1628864390263.png)
...
![1628864883564](images/1628864883564.png)
...
![1628865728305](images/1628865728305.png)
...
![1628867237329](images/1628867237329.png)
...
![1628867761245](images/1628867761245.png)

4.2 如果用户名和密码输入正确，则

![1628867801708](images/1628867801708.png)
...
![1628867904783](images/1628867904783.png)
